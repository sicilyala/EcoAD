...
class ActionType(object):
    ...
    def controlled_vehicle(self):
        ...
        # or: 从左到右计算表达式,返回碰到的第一个真值，如果都是假的值，则返回最后一个假的值
        return self.__controlled_vehicle or self.env.vehicle

    @controlled_vehicle.setter
    # 对property方法涉及的类内变量进行赋值，进而改变property的返回结果
...
class ContinuousAction(ActionType):
    ...
    # the default range of output power of FCHEV
    ENGINE_POWER_RANGE = (0, 100)  # kw
    ...
    def __init__(self,
                 ...
                 engine_power_range: Optional[Tuple[float, float]] = None,
                 longitudinal: bool = True,
                 lateral: bool = True,
                 ems_flag: bool = False,
                 dynamical: bool = False,  # False for Vehicle, True for BicycleVehicle
                 clip: bool = True,
                 **kwargs) -> None:
        """
        ...
        # print("\n-----here is in envs.common.action.py, acceleration_range is: ", self.acceleration_range,
        # " m/s2-----")
        self.steering_range = steering_range if steering_range else self.STEERING_RANGE
        # print("-----here is in envs.common.action.py, steering_range is: ", self.steering_range, " rad-----")
        self.speed_range = speed_range
        # print("-----here is in envs.common.action.py, speed_range is: ", self.speed_range, " m/s-----")
        self.engine_power_range = engine_power_range if engine_power_range else self.ENGINE_POWER_RANGE
        # print("-----EMS_flag is: ", EMS_flag, "-----")
        # print("-----here is in envs.common.action.py, engine_power_range is: ", self.engine_power_range, " kW-----\n")
        self.lateral = lateral
        self.longitudinal = longitudinal
        self.EMS_flag = ems_flag
        ...
        self.size = int(self.longitudinal + self.lateral + self.EMS_flag)
    ...
    def act(self, action: np.ndarray) -> None:
        if self.clip:
            action = np.clip(action, -1, 1, dtype=float)
        if self.speed_range:
            self.controlled_vehicle.MIN_SPEED, self.controlled_vehicle.MAX_SPEED = self.speed_range

        if self.lateral and self.EMS_flag:
            self.controlled_vehicle.act({
                "acceleration": utils.lmap(action[0], [-1, 1], self.acceleration_range),
                "steering": utils.lmap(action[1], [-1, 1], self.steering_range),
                "engine_power": utils.lmap(action[2], [0, 1], self.engine_power_range) # [0, 1] -> [0, 60] # [-0.5, 0, 0.5] -> [15, 30, 45] kw
            })
        elif self.lateral and not self.EMS_flag:
            self.controlled_vehicle.act({
                "acceleration": utils.lmap(action[0], [-1, 1], self.acceleration_range),
                "steering": utils.lmap(action[1], [-1, 1], self.steering_range),
                "engine_power": 0
            })
        elif not self.lateral and self.EMS_flag:
            self.controlled_vehicle.act({
                "acceleration": utils.lmap(action[0], [-1, 1], self.acceleration_range),
                "steering": 0,
                "engine_power": utils.lmap(action[1], [0, 1], self.engine_power_range)
            })
        else:
            self.controlled_vehicle.act({
                "acceleration": utils.lmap(action[0], [-1, 1], self.acceleration_range),
